...
//     const vhashedToken = await (await bcryptjs.hash(userId.toString(), 10)).slice(9, 15);
...
export const sendEmail = async ({ email, emailType, user }) => {
  try {
    const userId = user.id;
    const vhashedToken = await (
      await bcryptjs.hash(userId.toString(), 10)
    ).slice(9, 15);
    const fhashedToken = await (
      await bcryptjs.hash(userId.toString(), 10)
    ).slice(5, 13);

    if (emailType === 'VERIFY') {
      await User.findByIdAndUpdate(userId, {
        verifyToken: vhashedToken,
    ...
    const emailProps = {
      ...
      firstName: user.firstName,
      lastName: user.lastName,
      verificationToken: emailType === 'VERIFY' ? vhashedToken : fhashedToken,
    };
    ...
    await resend.emails.send({
      ...
      react:
        emailType === 'VERIFY' ? Email(emailProps) : ResetEmail(emailProps),
    });
